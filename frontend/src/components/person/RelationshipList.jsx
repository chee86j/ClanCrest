...
const RelationshipList = ({ personId = null }) => {
  ...
  const { relationships, loading, error, deleteRelationship } =
    useRelationships();
  ...
  useEffect(() => {
    ...
    if (personId) {
      setFilteredRelationships(
        relationshipsArray.filter(
          (rel) => rel.fromId === personId || rel.toId === personId
        )
      );
    } else {
      setFilteredRelationships(relationshipsArray);
    }
  ...
  // Ensure filteredRelationships is an array before mapping
  const safeFilteredRelationships = Array.isArray(filteredRelationships)
    ? filteredRelationships
    : [];

  if (safeFilteredRelationships.length === 0) {
    return (
  ...
  return (
    <div className="space-y-4">
      {safeFilteredRelationships.map((relationship) => (
        <div
          key={relationship.id}
          ...
          <div className="flex-1">
            <p className="text-sm text-gray-900">
              <span className="font-medium">
                {getPersonName(relationship.fromId)}
              </span>{" "}
              <span className="text-gray-500">
                {RELATIONSHIP_LABELS[relationship.type]}
              </span>{" "}
              <span className="font-medium">
                {getPersonName(relationship.toId)}
              </span>
            </p>
          </div>
